...
class IECTransformer(Transformer):
    ...
    def header_id_standard(self, tree):
        return (tree[0], tree[1], tree[2])

    def header_id_vendor(self, tree):
        return ("N/A", "N/A", "".join(tree))

    def header_id(self, tree):
        return tree[0]
    ...
    def id_cd(self, tree):
        return iec62056.objects.Reference(
            (None, None, int(tree[0]), int(tree[1]), None)
        )
    ...
    def profilegeneric(self, tree):
        ...
        return [
            iec62056.objects.Register.factory(reference, timestamp, value, unit)
            for timestamp, value, unit in entries
        ]
...
class Parser(object):
    def __init__(self):
        ...
        self.parser = Lark.open(grammar_file, start="telegrams")
