...
class Reference(object):
    ...
    def match(self, other):
        ...
        # run every digit
        for digit_self, digit_other in [
            (str(self.id[i]), str(other.id[i])) for i in range(len(self.id))
        ]:
...
class Log(COSEM):
    ...
    def factory(cls, reference, entries):
        ...
        raise NotImplementedError(
            "OBIS reference {} is not implemented".format(repr(reference))
        )

    def __repr__(self):
        return "<Log reference={} name={} [{}]>".format(
            self.reference, self.name, self.entries
        )
...
class Register(COSEM):
        ...
        for r, n, vcls in cosem_objects:
            if r.match(reference):
                try:
                    ...
                    logging.warning(
                        f"Invalid register: {cls, reference, timestamp, raw, unit}"
                    )
                    return None
                name = n
                return cls(reference, name, timestamp, value, unit)
        logging.warning("OBIS reference {} is not implemented".format(repr(reference)))
        return None
    ...
    def __repr__(self):
        if self.timestamp is not None:
            ...
            t = None
        return "<Register reference={} name={} timestamp={} value={} unit={}>".format(
            self.reference, self.name, t, self.value, self.unit
        )
...
class Telegram(object):
    ...
    def __repr__(self):
        return "<Telegram {}/{} containing {} checksum={}>".format(
            self.vendor, self.model, ",".join(self.keys()), self.checksum
        )
